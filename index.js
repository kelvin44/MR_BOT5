...
/******** OWNER NUMBER**********/
const ownerNumber = ["6281218305935@s.whatsapp.net"] 
/************************************/
...
const client = new WAConnection()
   ...
   console.log(color('[','white'),color('∆','red'),color(']','white'),color('qr already scan.subscribe','white'),color('YOU','red'),color('TUBE','white'),color('Anonym gaming','yellow'))
})
           ...
           return false
			...
			switch(command) { 
                ...
                case 'glitchtext':
					...
					var gli = gh.split("&")[0];
					var tch = gh.split("&")[1];
					if (args.length < 1) return reply(`「❗」Contoh : ${prefix}glitchtext kuntull & Gans`)
					reply(ind.wait())
...
• Limit: ${limitawal-updated.limit}`
           ...
           if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				case 'mining':
                      ...
                      if (isOwner) {
                      const one = 999999999999999999999999999
           ...
           case 'demote':
				...
				case 'bc': 
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `*「 MR BOT BORADCAST 」*\n\n${body.slice(4)}`)
						}
				...
				case 'bcgc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						buff = await client.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `*「 MR BOT BORADCAST 」*\n\nDari Grup : ${groupName}\nPengirim : wa.me/${(sender.split('@')[0])}\nPesan : ${body.slice(6)}`})
						}
						reply('')
					} else {
						for (let _ of anu) {
